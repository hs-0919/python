...
# print(x_fit)
y_lin_fit= model.predict(x_fit)  # 그래프 표시용으로 하나 만듬
# print(y_lin_fit)
...
# degree = 2 
model.fit(x_quad, y)
y_quad_fit= model.predict(quad.fit_transform(x_fit))  # 그래프 표시용으로 하나 만듬
q_r2 = r2_score(y, model.predict(x_quad))
print('q_r2 : ', q_r2)

# degree = 3 
model.fit(x_cubic, y)
y_cubic_fit= model.predict(cubic.fit_transform(x_fit))  # 그래프 표시용으로 하나 만듬
c_r2 = r2_score(y, model.predict(x_cubic))
print('c_r2 : ', c_r2)

# 시각화 
plt.scatter(x, y, c='pink', label='학습 데이터')
plt.plot(x_fit, y_lin_fit, linestyle='--', label='linear fit(d=1), $R^2=%.2f$'%model_r2, c='r', lw=3)
plt.plot(x_fit, y_quad_fit, linestyle='-', label='quad fit(d=2), $R^2=%.2f$'%q_r2, c='y', lw=3)
plt.plot(x_fit, y_cubic_fit, linestyle='dotted', label='cubic fit(d=3), $R^2=%.2f$'%q_r2, c='g', lw=3)
