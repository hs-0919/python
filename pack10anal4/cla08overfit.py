...
print()
# 분류 모델
dt_clf = DecisionTreeClassifier() # 분류 모델 중 하나 (디폴트값)
print(dt_clf)
...
# 예측 정확도 :  1.0  100%여서 뭔가 찝찝(?) -> 과적합 발생(오버피팅)
...
# print(x_train[:5])
# print(iris.data[:5])  랜덤하게 뽑은거라 두개가 같지가 않다.
dt_clf.fit(x_train, y_train)   # train data로 학습
pred2 = dt_clf.predict(x_test) # test data로 검정

print('예측값 : ', pred2)
print('실제값 : ', y_test)
print('예측 정확도 : ', accuracy_score(y_test, pred2)) # 예측 정확도 :  0.95555 - 100%가 되지 않도록 처리함: 모델의 포용성
# 100% 는 자기가 본것만 예측, 하지만 95%면 5%정도 다른것도 예측할 수 있다
# "추론통계는 모든 환경에서 100%가 되어서는 안된다."라는 철학을 이해하자!
# validation은 학습과 검증을 같이 한다. 

print('\n과적합 방지 방법2 : 교차검증(cross validation) - train/test split을 해도 과적합 발생 또는 데이터의 양이 적은경우 사용')
# 모델 학습도중 train data를 쪼개 validation data를 만든 후 학습시 검정을 함께함
# K-fold 교차검증 : train data를 k개(번) 접어 validation data로 사용.
...
cv_acc = []
print('iris shape : ', features.shape) # (150, 4) - 전체 150개
# 전체 행수 : 150개, 학습데이터 : 4/5(120개), 검증데이터 : 1/5(30개)

n_iter = 0 
for train_index, test_index in kfold.split(features):
    # print('n_iter: ', n_iter)
    # print('train_index: ', train_index) 120개
    # print('test_index: ', test_index) 30개
    # n_iter += 1
...
print('평균 검정 정확도: ', np.mean(cv_acc)) # 평균 검정 정확도:  0.91999

print('\n과적합 방지 방법2-1 : 교차검증(cross validation) - 불균형한 분포(편향, 왜곡된 데이터)를 가진 경우')
# 불균형이란 특정 레이블 값이 특이하게 많거나 적은 경우
from sklearn.model_selection import StratifiedKFold
skfold = StratifiedKFold(n_splits=5) # StratifiedKFold이거랑 iris는 맞지 않다. 하지만 보여주기용으로 사용

cv_acc=[]
n_iter=0

for train_index, test_index in skfold.split(features, label): # label을 준다는것에 차이가 있다. 나머지는 같음
    x_train, x_test = features[train_index], features[test_index]
    y_train, y_test = label[train_index], label[test_index]
    # 학습 및 예측
    dt_clf.fit(x_train, y_train)
...
data = iris.data
label = iris.target
score = cross_val_score(dt_clf, data, label, scoring='accuracy', cv=5) # estimator: 모델이 누구인가?, scoring: 너 뭐할거니~, cv:쪼개는거
print('교차 검증별 정확도 : ', np.round(score, 3))
print('평균 검정 정확도 : ', np.round(np.mean(score), 3)) # 평균 검정 정확도 :  0.953
...
grid_dtree = GridSearchCV(dt_clf, param_grid=para, cv=5, refit=True) # cv=? 이게 kfold 수 / refit-재학습
...
print('GridSearchCV 최적 정확도 : {0:.4f}'.format(grid_dtree.best_score_)) # 0.9417

# df_clf =DecisionTreeClassifier(max_depth=2, min_samples_split=2)
best_df_clf = grid_dtree.best_estimator_ # 최적의 모델로 파라미터를 만드는것
print(best_df_clf)
