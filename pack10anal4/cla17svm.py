...
print('Logistic 정확도(acc) : ', metrics.accuracy_score(label, pred)) # 정확도 :  0.75(75%)

print('-----'*30)
model2 =svm.SVC(C=1).fit(feature, label) # 선형 비선형 모두 가능, C=1 오버피팅 방지
# model2 =svm.LinearSVC().fit(feature, label) # LinearSVC() 이거도 가능하다.(SVC 계량종)


pred = model2.predict(feature)
print('svm 예측값 : ', pred)
print('svm 정확도(acc) : ', metrics.accuracy_score(label, pred)) # svm 정확도(acc) : 1.0
