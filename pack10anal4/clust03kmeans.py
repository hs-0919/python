...
from sklearn.datasets import make_blobs  # 연습용 data
plt.rc('font', family='malgun gothic')

x, y = make_blobs(n_samples= 150, n_features=2, centers=3, cluster_std=0.5, shuffle=True, random_state=0)
print(x[:3])
# print(y) -> 군집분석이므로 의미없다.

'''
plt.scatter(x[:, 0], x[:, 1], s=50, c='red', marker='o')
plt.grid(True)
plt.show()
'''

from sklearn.cluster import KMeans  # 비계층적 군집 분석 중 가장 많이 사용(데이터의 분포가 비선형인 경우 정확도가 떨어진다.)
# 최초의 군집 중심점을 설정하는 방법
# init_centroid ='random'
init_centroid ='k-means++'  # default값
# K-Means++는 전통적인 K-Means의 문제, 즉 중심점 무작위 선정의 문제를 해결하기 위해 생겨난 것이라고 생각하면 이해하기 편하다.
...
print(x[pred == 1][:3])
print(x[pred == 2][:3])
print('centroid : ', kmodel.cluster_centers_)  # 군집 중심점
...
plt.scatter(x[pred == 2, 0], x[pred == 2, 1], s=50, c='yellow', marker='v', label='cluster3')
plt.scatter(kmodel.cluster_centers_[:, 0], kmodel.cluster_centers_[:, 1], 
            s=70, c='black', marker='+', label='center') # 중심점
plt.grid(True)
plt.legend()
plt.show()

# K-Means에서 K를 정하는 것이 가장 중요한 문제 중 하나이다.
# 방법1 : 계층적 군집으로 K값 유추
# 방법2 : 엘보우(elbow) 기법 - 클러스터 간 SSE(제곱오차)의 차이를 이용
def elbow(x):
    sse = []
    for i in range(2, 11):
...
# 방법3 : 실루엣(silhouette) 기법 - 클러스터 간 실루엣 계수값을 수평 막대 그래프로 표현
