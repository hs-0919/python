# Logistic Regression
# 종속변수 와 독립변수 간의 관계로 예측모델을 생성한다는 점에서 선형회귀분석과 유사하다 하지만
# 독립변수 (x) 에 의해 종속변수 (y)의 범주로 분류한다는 측면에서 분류분석 방법이다.
# 분류 문제에서 선형예측에 시그모이드 함수를 적용하여 가능한 각 불연속 라벨 값에 대한 확률을 생성하는
...
# 연비와 마력수에 따른 변속기 분류 (자동 ,수동)
# 모델 작성 1 : logit()
import statsmodels.formula.api as smf

formula = 'am ~ hp + mpg'
result = smf.logit(formula=formula, data=mtcar).fit()
print(result) # Iterations 9 - 9번 학습함
print(result.summary())
# print('예측값 : ', result.predict())

pred =result.predict(mtcar[:10])
print('예측값 : ', pred.values)
print('예측값 : ', np.around(pred.values)) # np.around() - 0.5를 기준으로 0과1로 출력
print('실제값 : ', mtcar['am'][:10].values)

print()
conf_tab = result.pred_table() # confusion matrix 혼돈행렬
print(conf_tab)
print('분류정확도 : ', (16+10)/len(mtcar)) # 분류정확도 :  0.8125
print('분류정확도 : ', (conf_tab[0][0] + conf_tab[1][1])/len(mtcar)) # 분류정확도 :  0.8125
from sklearn.metrics import accuracy_score
pred2 = result.predict(mtcar)
print('분류정확도 : ', accuracy_score(mtcar['am'], np.around(pred2)))# 분류정확도 :  0.8125
...
print(newdf)
newdf['mpg'] = [10, 20, 30, 40, 50]
newdf['hp'] = [100, 120, 130, 140, 150]
print(newdf)
