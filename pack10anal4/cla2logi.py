...
print(data.head(4), data.shape) # (366, 12) RainTomorrow
data2 = pd.DataFrame()
...
print('conf_mat : \n', conf_mat) # glm은 안돼~~~ / logit는 가능- pred_table()만 가능
print('분류 정확도 : ', (conf_mat[0][0] + conf_mat[1][1]) / len(train)) # 분류 정확도 :  0.87109375
from sklearn.metrics import accuracy_score
pred = model.predict(test)
print('분류 정확도 : ', accuracy_score(test['RainTomorrow'], np.around(pred))) # 분류 정확도 :  0.87272727
