# 다항분류(얘는 활성화 함수 softmax 사용 - 결과값을 확률로 반환)
# softmax 함수를  사용하기 때문에 
...
# model = LogisticRegression(C=1.0, random_state=0, verbose=0)  # C=1.0 L2규제(패널티 적용-오버피팅) - 값이 작을수록 규제는 강화된다.
...
# 분류정확도(accuracy) - 1
print('%.5f'%accuracy_score(y_test, y_pred)) #  0.95556

# 분류정확도(accuracy) - 2
con_mat = pd.crosstab(y_test, y_pred, rownames=['예측치'], colnames=['관측치'])
print(con_mat)
print((con_mat[0][0] + con_mat[1][1] + con_mat[2][2]) / len(y_test)) # 0.95555

# 분류정확도(accuracy) - 3
print('test : ', model.score(x_test, y_test))    # 0.95555
print('train : ', model.score(x_train, y_train)) # 0.99047
...
dot_data = StringIO() # StringIO() - 파일 흉내를 내는 역할
