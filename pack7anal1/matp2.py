# 차트의 종류 경험하기 : data의 성격에 따라 차트를 적용하는 것이 중요하다.
...
#
# ax1. hist(np.random.randn(10), bins=5, alpha = 0.9) # alpha 투명도
# ax2.plot(np.random.randn(10))
# plt.show()


# data = [50, 80, 100, 70, 90]  # 세로 막대 그래프
# plt.bar(range(len(data)), data)
# plt.show()
#
# data = [50, 80, 100, 70, 90]  # 가로 막대 그래프
# err = np.random.rand(len(data))
# plt.barh(range(len(data)), data, xerr=err)
# plt.show()

# data = [50, 80, 100, 70, 90] 
# plt.pie(data, explode=(0, 1, 0.1,0,0), colors=['yellow', 'blue', 'red']) # explode 조각 분리
# plt.show()
#
# plt.boxplot(data)
# plt.show()
